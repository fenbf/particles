...
//#define UTILS_LOG_WITH_OUTPUT_DBG_STRING
    ...
    #define LOG(msg, ...)         { logger::winOutDebugString(false, 0, 0, 0, 0, msg, ##__VA_ARGS__); }
    #define LOG_SUCCESS(msg, ...) { logger::winOutDebugString(false, 0, 0, 0, "SUCCESS", msg, ##__VA_ARGS__); }
    #define LOG_ERROR(msg, ...)   { logger::winOutDebugString(true, __FUNCTION__, __FILE__, __LINE__, "ERR", msg, ##__VA_ARGS__); }
#else
